...
class ResizeMultiViewImagesNapLab(object):
    ...
    def __call__(self, results:dict):
        ...
        for img,  cam_intrinsic, ego2img, ego2cam, cx, cy, fw_coeff in zip(results['img'], \
                results['cam_intrinsics'], results['ego2img'], results['ego2cam'], results['cx'], results['cy'], results['fw_coeff']):
            if self.scale is not None:
                h, w = img.shape[:2]
                target_h = int(h * self.scale)
                target_w = int(w * self.scale)
