...
def plot_one_frame_results_sample_image(vectors, id_info, roi_size, scene_dir, index, sample_image, args):                
    # setup the figure with car
    fig, ax = plt.subplots(figsize=(roi_size[0], roi_size[1]))
    ax.set_xlim(-roi_size[0] / 2, roi_size[0] / 2)
    ax.set_ylim(-roi_size[1] / 2, roi_size[1] / 2)
    ax.axis('off')
    plt.autoscale(False)
    car_img_r = Image.open('/cluster/home/terjenf/maptracker/resources/car-orange.png')
    ...
    file_location = os.path.join(scene_dir, "pred")
    ...
    ax_inset.imshow(sample_image)
    ax_inset.axis('off') 
        
    save_path = os.path.join(file_location, f'sample_{index}.png')
...
def plot_one_frame_results(vectors, id_info, roi_size, scene_dir, index, args):                
    # setup the figure with car
...
def vis_pred_data(scene_name, args, pred_results, origin,roi_size):
    ...
    for index in index_list:
        ...
        viz_image = plot_one_frame_results_im(curr_vectors, id_info, roi_size, scene_dir, index, args)
...
def plot_sample_canvas(cam_dict, ind, scene_dir, save): 
    ...
    for cam_name, v in cam_dict.items():
        ...
        axis[cam_layout[cam_name]].imshow(Image.open(v))
    ...
    if save: 
        file_location = os.path.join(scene_dir, "camera")
        if not os.path.exists(file_location):
            os.makedirs(file_location)

        filename = os.path.join(file_location, f"camera_sample_{ind}.png")
        plt.tight_layout()
        fig.suptitle(f" sample index: {ind}")
...
def vis_pred_data_image(scene_name, args, pred_results, origin,roi_size):

    dataroot ="/cluster/home/terjenf/naplab/data"
    trip="Trip077"
    naplab_77 = NapLab(dataroot=dataroot, trip=trip)
    ...
    for index in index_list:
        ...
        sample = pred_results[index]
        sample_idx = sample['meta']['sample_idx']

        sample_naplab = naplab_77.sample[sample_idx]


        # filenames = {}
        # for cam, sample_data_token in sample_naplab['data'].items():
        #     sample_data = naplab_77.get('sample_data', sample_data_token)
        #     img_file = sample_data['filename']
        #     filenames[cam] = img_file

            
        sample_image = naplab_77.visualize_sample_compact(sample_idx, model_name="Maptracker", save=True)
        ...
        viz_image = plot_one_frame_results_sample_image(curr_vectors, id_info, roi_size, scene_dir, index, sample_image, args)
...
def main():
    ...
    for scene_name in all_scene_names:
        ...
        if args.option == "vis-gt":
            ...
            vis_pred_data_image(scene_name=scene_name, args=args, pred_results=data, origin=origin, roi_size=roi_size)
